...
const LS="chemquest-organic-expanded-v2";
let state=JSON.parse(localStorage.getItem(LS)||'{"tries":{},"correct":{},"wrong":{},"selectedUnit":"u01"}');
let sets={qa:[],tf:[],choice:[],route:[]}, idx=0, activeMode="qa";
...
function overall(){const tries=Object.keys(state.tries).length, correct=Object.keys(state.correct).length, weak=Object.keys(state.wrong).length, rate=tries?Math.round(correct/tries*100):0;return{tries,correct,weak,rate}}
...
function start(mode){activeMode=mode;state.selectedUnit=$("#unitSelect").value||state.selectedUnit;sets[mode]=limit(byUnit(mode,state.selectedUnit));idx=0;showPage(mode); if(mode==="qa")showQa(); if(mode==="tf")showTf(); if(mode==="choice")showChoice(); if(mode==="route")showRoute();}
